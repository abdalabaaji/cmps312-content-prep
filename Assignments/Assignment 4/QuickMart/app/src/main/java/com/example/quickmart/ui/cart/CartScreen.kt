...
fun CartScreen(onCheckOut: () -> Unit) {
    ...
    val productsInfo by cartViewModel.productsInfo.collectAsState()
    ...
    QuickMartTheme {
        Scaffold(topBar = {
            ...
            CartItemsList(
                cartItems,
                productsInfo,
                { cartViewModel.updateCart(it.productId, -1) },
                { cartViewModel.updateCart(it.productId, 1) },
                { cartViewModel.updateCart(it.productId, -it.quantity) },
                Modifier.padding(it)
            )
...
fun CartItemsList(
    ...
    productsInfo: List<Pair<Product, Double>>,
...
fun CartItemCard(
    item: CartItem,
    productInfo: Pair<Product, Double>,
    onDecreaseQuantity: (CartItem) -> Unit,
    onIncreaseQuantity: (CartItem) -> Unit,
    onRemoveCartItem: (CartItem) -> Unit
) {
    val price = productInfo.second
    val product = productInfo.first
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Icon(Icons.Default.Add,
                            ...
                            modifier = Modifier
                                ...
                                .padding(6.dp)
                                .clickable { onIncreaseQuantity(item) })
                    }
                    Text(
